...
namespace RGR
{
    public partial class ReferenceSalesProdavec : Form
    {
        MainForm mainForm;
        public ReferenceSalesProdavec()
        {
            InitializeComponent();
        }
        public ReferenceSalesProdavec(MainForm main)
        {
            InitializeComponent();
            mainForm = main;
            addDataComboBox();
        ...
        {
            string comand = "SELECT * FROM Сотрудники ORDER BY ИдентификаторСотрудника;";
            DataTable dt = new DataTable();
            dt = mainForm.GetData(comand);
            dt.Columns.Add("ФамилияИмяНомер", typeof(string), "Фамилия + ' ' + Имя + '(' + НомерСотрудника + ')'");
            comboBox1.DataSource = dt;
            comboBox1.DisplayMember = "ФамилияИмяНомер";
        ...
        private void buttonAccept_Click(object sender, EventArgs e)
        {
            if (comboBox1.SelectedItem != null)
            {
                ...
                string comand = "SELECT * FROM Справка_о_продажах_продавца(" + comboBox1.SelectedValue.ToString() + ");";
                DataTable dt = new DataTable();
                dt = mainForm.GetData(comand);
        ...
        private void buttonPrint_Click(object sender, EventArgs e)
        {
            ...
            DataGridViewPrinter printer = new DataGridViewPrinter(dataGridView1, rowView.Row[2].ToString());
        ...
        public class DataGridViewPrinter
        {
            ...
            private void PrintPage(object sender, PrintPageEventArgs e)
            {
                ...
                // Печать заголовка
                string header = "Справка о продажах продавца " + selTable;
